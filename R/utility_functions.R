...
calcLargeScaleEvents <- function()
{
    i<- 1
    object <- final.objects[[i]]
    data <- object@control.normalized[[3]]

    l<- apply(data[,], 2, function(x) unlist(lapply(split(x, object@annotation.filt$cytoband), mean) ))
    thr <- quantile(l[,], prob=c(0.01, 0.99))

    finalChrMat2 <- l
    finalChrMat2[l<as.numeric(thr[1])] <- (-1)
    finalChrMat2[l>as.numeric(thr[2])] <- 1
    finalChrMat2[l>=as.numeric(thr[1]) &l<=as.numeric(thr[2])] <- 0

    finalChrMat2 <- t(finalChrMat2)
   
}
    # mod4 <- densityMclust(na.omit(as.numeric(l)), modelName = "V", verbose = F)
    # min_thr <- median(mod4$data[mod4$classification == 1])
    # max_thr <- median(mod4$data[mod4$classification == dim(table(mod4$classification))])


    # finalChrMat3 <- l
    # finalChrMat3[l<as.numeric(min_thr)] <- (-1)
    # finalChrMat3[l>as.numeric(max_thr)] <- 1
    # finalChrMat3[l>=as.numeric(min_thr) &l<=as.numeric(max_thr)] <- 0
    # finalChrMat3 <- t(finalChrMat3)
  
   

    # dim(table(mod4$classification))
    # table(finalChrMat2[, "1p" ], finalChrMat[, "1p" ] )
    
    # table(finalChrMat3[, "1p" ], finalChrMat[, "1p" ] )

    # l[finalChrMat==(-1)] 

    #     l[ "1p",finalChrMat[, "1p"]==(-1)]

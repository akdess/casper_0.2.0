...
CreateCasperObject <- function(raw.data, annotation, control.sample.ids, cytoband, loh.name.mapping, cnv.scale, loh.scale,
    ...
    object <- new(Class = "casper", raw.data = raw.data, loh = loh, annotation = annotation, sequencing.type = sequencing.type,
        ...
        cnv.scale = cnv.scale, loh.scale = loh.scale, matrix.type = matrix.type, method = method, window.length = window.length, 
        length.iterations = length.iterations,
         genomeVersion = genomeVersion, filter = filter)


    object.raw.data <- object@raw.data
    object@raw.data <- NULL
   ...
   object@data <- NULL
    ...
    centered.data <- AverageReference(data = centered.data, ref_ids = object@control.sample.ids)
    lower_bound <- mean(apply(centered.data, 2,
                              function(x) quantile(x, na.rm=TRUE)[[1]]))
    upper_bound <- mean(apply(centered.data, 2,
                              function(x) quantile(x, na.rm=TRUE)[[5]]))

    threshold = mean(abs(c(lower_bound, upper_bound)))

    centered.data[centered.data > threshold] <- threshold
    centered.data[centered.data < (-1 * threshold)] <- -1 * threshold
    #rm(list=c("object.data", "object.raw.data"))
    object <- ProcessData(object, centered.data)
...
#'
#'
ProcessData <- function(object, centered.data) {

    if (object@filter=="median")   object <- PerformMedianFilterByChr(object, centered.data)
    if (object@filter=="mean")   object <- PerformMeanFilterByChr(object, centered.data)
...
PerformMedianFilter <- function(object, centered.data, window.length = 50, length.iterations = 50) {

    filtered.data <- list()

    for (i in 1:object@cnv.scale) {
        if (i == 1) {
            filtered.data[[i]] <- apply(centered.data, 2, function(x) round(filter(MedianFilter(window.length +
                1), x), digits = 2))
            rownames(filtered.data[[i]]) <- rownames(centered.data)
...
PerformMeanFilterByChr <- function(object, centered.data) {
    ...
    for (i in 1:object@cnv.scale) {
        ...
        filtered.data[[i]] <- filtered.data[[i-1]]
          ...
          row_median <- apply(filtered.data[[i]], 2, function(x) { median(x, na.rm=TRUE) } )
         filtered.data[[i]]<- t(apply(filtered.data[[i]], 1, "-", row_median))
         filtered.data[[i]] <- AverageReference(data = filtered.data[[i]], ref_ids = object@control.sample.ids)

  
    }

    object@control.normalized <- filtered.data
    return(object)
}
...
PerformMedianFilterByChr <- function(object, centered.data=centered.data) {
...
ControlNormalize <- function(object) {
  ...
  #  object@control.normalized.noiseRemoved <- list()
    ...
    for (i in 1:object@cnv.scale) {
        object@control.normalized[[i]] <- AverageReference(data = object@filtered.data[[i]], ref_ids = object@control.sample.ids)
...
AverageReference <- function(data, ref_ids) {
    ...
    grp_average <- Matrix::rowMeans(average_reference_obs, na.rm = TRUE)
